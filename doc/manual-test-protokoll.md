...
| **Result** | PASS / FAIL — Date — Tester — Notes: BUG-44 (preset dropdown), BUG-47 (wizard 2.2 GB), BUG-48 (batch_size now propagates correctly), BUG-49 (HTTP 422 missing name field + no dataset selection + error message overflow), BUG-50 (Redis/Celery dependency — training fails with 500 when Redis not running), BUG-51 (button has no loading state/feedback) |
