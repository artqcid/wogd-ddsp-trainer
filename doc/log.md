...
- **BUG-43** `batch_size` is now VRAM-dependent: `batch_size_max = min(128, max(2, int(vram × 32/6)))` computed dynamically from GPU VRAM; `propose_presets()` scales batch_size by speed factor (FAST ×0.25, NORMAL ×0.50, QUALITY ×1.00); `clamp_params()` clamps it; `apply_speed()` scales it like hidden_size; DataLoader reads batch_size from run config instead of hardcoded 1.
